...
**결론: 낮은 AUC는 모델/신호 문제가 아니라 라벨 노이즈였다.** SolRPDS "Active"는 "아직 안 죽음"일
뿐 검증된 정상이 아니라(pump.fun 98.6%가 결국 스캠), 러그 예비군이 섞여 러그와 안 갈렸다. 애매한
경계를 빼고 **확실한 러그 vs 확실한 생존자만 보면 홀더집중만으로 AUC 0.90.** (단 (C)는 정상을 활동량으로
정의해 다소 쉬운 분할 = 상한 추정.) 홀더집중은 **명확한 케이스는 잘 가르고 애매경계에서만 약하다.**

### 🧭 종합 결론
1. **높은 정확도(0.94~0.99)는 항상 부정행위 신호였다** — 누수(타임스탬프·제거유동성) 또는 편향
   (memecoin vs 블루칩). 정직하게 통제하면 애매표본 AUC ~0.78, 깨끗한 라벨이면 ~0.90.
