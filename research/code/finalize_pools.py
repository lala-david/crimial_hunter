...
vd = list(csv.DictReader(open(os.path.join(D, "..", "data", "pool_verdict.csv"), encoding="utf-8")))
...
with open(os.path.join(D, "..", "data", "false_positive_pools.txt"), "w", encoding="utf-8") as fo:
    fo.write("\n".join(r["pool"] for r in fp))
with open(os.path.join(D, "..", "data", "scam_mints_from_pools.txt"), "w", encoding="utf-8") as fo:
